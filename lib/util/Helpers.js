...
(function() {
  ...
  Helpers = (function() {
    ...
    Helpers.prototype.trace = function(err) {
      var error, options;
      options = {};
      if (err.trace) {
    ...
    Helpers.prototype.fetchHookMethod = function(hook_name) {
      var bundled, combined_modules, modules;
      ...
      return _.find(_.compact(combined_modules), {
        name: hook_name
      });
    };
    ...
    Helpers.prototype.run = function(command, hooks_to_proccess, config, args, cb) {
      var hooks_methods, started_at, x;
      x = 0;
      started_at = new Date();
      if (config && config.config.hooks_to_ignore) {
        hooks_to_proccess = _.filter(hooks_to_proccess, function(hook) {
          return !_.contains(config.config.hooks_to_ignore, hook.name);
        });
      }
    ...
    Helpers.prototype.parseCommand = function(command) {
      var parsed_opts;
      parsed_opts = {};
      if (command[0]) {
